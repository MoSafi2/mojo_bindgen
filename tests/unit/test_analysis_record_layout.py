...
def test_record_layout_reports_fma_union_member_layout_as_empty_inline_array() -> None:
    holder = Struct(
        decl_id="struct:Holder",
        name="Holder",
        c_name="Holder",
    ...
    assert facts.plain_fields == ()
    assert facts.layout_problems == ("field `fma_array` has unsupported layout metadata",)
