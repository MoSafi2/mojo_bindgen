...
from mojo_bindgen.ir import Pointer, Primitive, PrimitiveKind, Qualifiers
from mojo_bindgen.codegen.lowering import lower_type


def test_lower_void_pointer_mutable_external() -> None:
    t = Pointer(pointee=None, qualifiers=Qualifiers(is_const=False))
...
def test_lower_void_pointer_const_external() -> None:
    t = Pointer(pointee=None, qualifiers=Qualifiers(is_const=True))
    s = lower_type(t, ffi_origin="external")
...
def test_lower_const_int_pointer_external() -> None:
    ...
    t = Pointer(pointee=ip, qualifiers=Qualifiers(is_const=True))
    s = lower_type(t, ffi_origin="external")
