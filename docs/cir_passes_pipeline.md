...
This ensures later lowering can still emit opaque record stubs for externally
referenced or transitively used record types.
...
- synthesized incomplete record declarations for reachable but non-emitted refs
...
So the CIR pass layer is intentionally narrow: validate, materialize reachable
records, canonicalize, then hand off to MojoIR lowering.
