    ...
    def run(
        ...
        *,
        ...
        if should_emit_align:
            align_decorator_value = ab
            align_decorator = f"@align({ab})"
            decorator_lines.append(align_decorator)
            if decl.size_bytes % ab != 0:
                align_stride_warning = True
                decorator_lines.append(
                    "# FFI: array stride follows size_of[T](); only T[0] is guaranteed align_of[T]-aligned; pad struct size to a multiple of alignment for per-element alignment."
                )
        ...
        if decl.is_packed:
            ...
            decorator_lines.append(packed_comment)
            align_omit_comment = packed_comment if align_omit_comment is None else f"{align_omit_comment} {packed_comment[2:]}"
        ...
        return AnalyzedStruct(
            ...
            emit_fieldwise_init=init_kind == "fieldwise" and not has_atomic_storage and decl.is_complete,
            fields=fields,
