...
class RecordLowerer:
    ...
    def _lower_field_type(self, cursor: cx.Cursor) -> tuple[Type, list[Struct]]:
        attached = self._attached_inline_record_definition(cursor)
        if attached is not None:
            nested_defs, inner = self.lower_record_definition(attached)
            all_nested = nested_defs + [inner]
            if self._field_type_matches_record(cursor, attached):
                return self.make_struct_ref(inner), all_nested
            return self.type_lowerer.lower(cursor.type, TypeContext.FIELD), all_nested
        return self.type_lowerer.lower(cursor.type, TypeContext.FIELD), []

    @staticmethod
    def _attached_inline_record_definition(field: cx.Cursor) -> cx.Cursor | None:
        """Return one inline record definition attached directly to a field."""
    ...
    def _field_type_matches_record(self, field: cx.Cursor, record: cx.Cursor) -> bool:
        """Return whether a field's direct canonical type is the attached record."""
